...
const RegisterAWSInfo = () => {
  ...
  const setBubbleStyles = (bubbles) => {
    for (let i = 0; i < bubbles.length; i++) {
      ...
      bubbles[i].style.animationDuration =
  ...
  const [awsAccountName, setLocalAccountName] = useState(
    register.awsAccountName
  );
  const [awsAccountType, setLocalAccountType] = useState(
    register.awsAccountType
  );
  ...
  const redirectionCloudFomrationTemplateSummaryPage = useCallback(() => {
    const templateURL =
      "https://cloudops-tenants-cf-template.s3.ap-northeast-2.amazonaws.com/cloudops_tenants_cf_template.yaml";
  ...
  useEffect(() => {
    ...
    root.style.background =
      "transparent linear-gradient(128deg, #754400 0%, #333354 100%) 0% 0% no-repeat padding-box";
      ...
      <div className="getstarted-cont">
        ...
        <div className="getstarted-cont__form">
          ...
          <div className="getstarted-cont__form-bottom">
            <p className="getstarted-cont__form-title">
              IAMPAM Company Info Registration
            </p>
                    ...
                    onChange={(e) =>
                      setLocalAWSRegions((prev) => {
                        if (Array.isArray(prev) && prev.length)
                          return prev.indexOf(e.target.value) < 0
                            ? [...prev, e.target.value]
                            : prev;
                      })
                    }
